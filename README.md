...
-   处理效率：
    ...
    -   显然，定长编码的处理效率要高于变长编码，但定长编码的空间复杂度可能稍高；

    -   在所有变长编码中，UTF-8的处理效率较高，这是因为UTF-8使用的编码方式可以通过CPU的位移运算器高效处理；而且对于英文字符占比较高的文本（例如HTML），UTF-8的空间利用率也较高；
...
-   由于不同编码之间不存在逻辑关系，所以编码转换的主要方式是“查表法”；

-   很多平台在存储和处理文本时，会将文本转换为一种“中间编码”，要求这种中间编码符合“效率高、空间利用率高”的特点，UCS-2编码较为符合，所以.net，Java，Windows平台均选用其做为最底层的字符编码；Linux，Unix系统则默认使用UTF-8编码和UCS4编码；

-   一般平台都会提供编码转换的API，在处理“字符串”，“字符流”时，对编码进行转换。
